...
# import conslib.screen as sc
# from console.utils import cls
# from conslib.style import fg, bg
# import console
import os
try:
...
class BASICParser:
    ...
    def __init__(self, basicdata):
        ...
        # screen mode
        self.__screenmode = None
        self.__screensize_X = None
        self.__screensize_Y = None

        # cursor location
        self.__screencol = self.__prnt_column
        self.__screenline = None
    ...
    def __simplestmt(self):
        ...
        elif self.__token.category == Token.SCREEN:
            self.__screenstmt()
            return None
        ...
        elif self.__token.category == Token.LOCATE:
            self.__locatestmt()
            return None
            
        elif self.__token.category == Token.COLOR:
            self.__colorstmt()
            return None
    ...
    def __printstmt(self):
            ...
            if prntTab:
                ...
                if current_pr_column > 1:
                    if fileIO:
                        ...
                        # screenMode = False
                        # self.__screenline = None  
                        screenText += format(" "*(current_pr_column-1))              
                    else:
                ...
                if fileIO:
                    ...
                    # screenText += format(self.__operand_stack.pop(), end='')   
                    tmp1 = self.__operand_stack.pop()
                    # str_tmp = format(tmp1, end='')       
                    screenText += tmp1         
                else:
            ...
            while self.__token.category == Token.SEMICOLON:
                ...
                if prntTab:
                    ...
                    if fileIO:
                        ...
                        screenText += self.__operand_stack.pop()
                    else:
                        print(self.__operand_stack.pop(), end='')
        ...
        if fileIO:
            ...
            self.locate(self.__screencol, self.__screenline, screenText)
            screenMode = False
            self.__screenline = None
            self.__screencol = None
    ...
    def __locatestmt(self):
        # parse LOCATE statement 

        if self.__screenmode == None:
            # invalid mode
            raise RuntimeError("Screen mode is not set for LOCATE cursor in line " + str(self.__line_number))
            
        self.__advance()   # Advance past LOCATE token
         
        #locate parameters are separated by comma, up to two parameters
        ...
        if self.__token.category == Token.COMMA:
            ...
            if not self.__tokenindex >= len(self.__tokenlist):
                ...
                if X > self.__screensize_X:
                    raise RuntimeError("Screen size is smaller than X coordinate for LOCATE cursor in line " + str(self.__line_number))
        ...
        if self.__token.category == Token.COMMA:
            ...
            if not self.__tokenindex >= len(self.__tokenlist):
                ...
                if Y > self.__screensize_Y:
                    raise RuntimeError("Screen size is smaller than Y coordinate for LOCATE cursor in line " + str(self.__line_number))
                
                else:
                    self.__screenline = Y
            else:
                raise SyntaxError("Missed arguments in line "+ str(self.__line_number))
            
        # all coordinates is counted
        # self.locate(X, Y, "*")
    ...
    def __beepstmt(self):
        # Beep
        # this can be function BEEP() and this can be keyword: BEEP
        self.__advance()  # Advance past SCREEN token
        l = len(self.__tokenlist)
        if l == 1:
            self.beepsound()
        elif l == 3:
            ...
            raise SyntaxError("Invalid argument for CLS in line "+ str(self.__line_number))
        
    def __clsstmt(self):
        # clear screen
        # this can be function CLS() and this can be keyword: CLS
        self.__advance()  # Advance past SCREEN token
        l = len(self.__tokenlist)
        if l == 1:
            self.clearscreen()
    ...
    def __screenstmt(self):
        # Parse screen statement
        self.__advance()  # Advance past SCREEN token
        self.__expr()
...
#            print("Text Mode")
            ...
            raise SyntaxError('Syntax error: SCREEN mode can be 0nly 0 or 1' +
                                  ' in line ' + str(self.__line_number))
        
        return      
        
    def ColorValToAnsi(self, Val,vFG):
        if   Val < 0 or Val > 7:
            raise RuntimeError("Invalid COLOR values in line " + str(self.__line_number))
        
        if Val == 0:
            if vFG:
                return '\x1b['+str(30) + 'm' #fg.black
            else:
                return '\x1b['+str(40) + 'm'
        elif Val == 1:
            if vFG:
                return '\x1b[' +str(34) + 'm'       # fg.blue
            else:
                return '\x1b[' +str(44) + 'm'
        elif Val == 2:
            if vFG:
                return '\x1b[' +str(32) + 'm'        #FG.GREEN  
            else:
                return '\x1b[' +str(42) + 'm'
        elif Val == 3:  
            if vFG:
                return '\x1b[' +str(36) + 'm'        #fg,cyan
            else:
                return '\x1b[' +str(46) + 'm'
        elif Val == 4:
            if vFG:
                return '\x1b[' +str(31) + 'm'        # fg.red
            else:
                return '\x1b[' +str(41) + 'm'        
        elif Val == 5:
            if vFG:
                return '\x1b[' +str(35) + 'm'        # fg.magenta
            else:
                return '\x1b[' +str(45) + 'm'
        elif Val == 6:
            if vFG:
                return '\x1b[' +str(33) + 'm'        # fg.yellow
            else:
                return '\x1b[' +str(43) + 'm'
        else:
            if vFG:
                return '\x1b[' +str(37) + 'm'        # fg.white
            else:
                return '\x1b[' +str(47) + 'm'
           
    def __colorstmt(self):
        # color statement
        
        if self.__screenmode == None:
            # invalid mode
            raise RuntimeError("Screen mode is not set for COLOR in line " + str(self.__line_number))
        ...
        if self.__token.category == Token.COMMA:
            ...
            if not self.__tokenindex >= len(self.__tokenlist):
                self.__expr()
                ValNum = self.__operand_stack.pop()
                FG = self.ColorValToAnsi(ValNum, True)
                
        if FG != None:
            print(FG, end ='')       #set foreground color
        ...
        if self.__token.category == Token.COMMA:
            ...
            if not self.__tokenindex >= len(self.__tokenlist):
                self.__expr()
                ValNum = self.__operand_stack.pop()
                BG = self.ColorValToAnsi(ValNum, False)
            else:
                raise SyntaxError("Missed arguments in line "+ str(self.__line_number))
            
        if BG != None:
            print(BG, end ='')       #set foreground color
             
    def __pausestmt(self):
        # PAUSE statement
        self.__advance()   # Advance past LOCATE token
    ...
    def __eofstm(self):
        ...
        if self.__file_handles.get(filenum) == None:
            raise RuntimeError("EOF: file #"+str(filenum)+" not opened in line " + str(self.__line_number))

        return self.is_eof(self.__file_handles[filenum])
    ...
    def __ifstmt(self):
        ...
        #Check for EOF()
        if self.__token.category == Token.EOF:
            saveval = self.__eofstm()
        else: 
            self.__logexpr()
    ...
    def __evaluate_function(self, category):
        ...
        if category == Token.INKEY:
            return msx.inkey_impl()
